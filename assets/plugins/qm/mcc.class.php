<?php
/**
 * QuickManager+ Manager Control Class
 *  
 * @author      Mikko Lammi, www.maagit.fi
 * @license     GNU General Public License (GPL), http://www.gnu.org/copyleft/gpl.html
 * @version     1.5.5 updated 02/08/2010
 */

class Mcc {
    public $head;
    private $script = array();
    private $tabs;
    private $sections;
    private $fields;
    //_______________________________________________________
    function __construct() {
        $this->tabs = array(
            'general'  => array('index'=>1, 'id'=>'tabGeneral'),
            'settings' => array('index'=>2, 'id'=>'tabSettings'),
            'access'   => array('index'=>3, 'id'=>'tabAccess')
        );

        $this->sections = array(
            'docsettings' => array('index'=>0, 'name'=>'DocSettings'),
            'content'     => array('index'=>1, 'name'=>'Content'),
            'tvs'         => array('index'=>2, 'name'=>'TVs')
        );

        $this->fields = array('content','pagetitle','longtitle','menuindex','parent','description','alias','link_attributes','introtext','template','menutitle');
    }

    //_______________________________________________________
    function addLine($line) {
        $this->script[] = $line;
    }

    //_______________________________________________________
    function Output() {
        $out = $this->head;
        $this->addLine('document.body.style.display="block";');
        $out.= sprintf(
            '<script type="text/javascript">jQuery(function(){%s});</script>'
            , implode("\n", $this->script)
        );

        return $out;
    }

    // Template
    //_______________________________________________________
    function hideTemplate($tpl) {
        $this->addLine('jQuery("select#template option[value='.$tpl.']").remove();');
    }

    //_______________________________________________________
    function hideTemplates($tpls) {
        if(is_array($tpls)) {
            foreach($tpls as $tpl) {
                $this->hideTemplate($tpl);
            }
            $this->hideTemplate(0); // remove blank
        }
        else {
            $this->hideTemplate(0); // remove blank
        }
    }

    // Section
    //_______________________________________________________
    function hideSection($section) {
        if(!isset($this->sections[$section])) return;
        $sectionBodyIndex = $this->sections[$section]['index'];
        $sectionHeaderIndex = $sectionBodyIndex -1;

        // Handle docsettings
        if ($sectionHeaderIndex == -1) {
            $this->addLine('jQuery("#tabGeneral table:eq(0)").hide()');
            return;
        }

        $this->addLine(
            sprintf(
                'jQuery("div.sectionHeader:eq(%d)").hide()'
                , $sectionHeaderIndex
            )
        );
        $this->addLine(
            sprintf(
                'jQuery("div.sectionBody:eq(%s)").hide()'
                , $sectionBodyIndex
            )
        );
    }

    // Tab
    //_______________________________________________________
    function hideTab($tab) {
        $tabIndex = $this->tabs[$tab]['index'];
        $tabId = $this->tabs[$tab]['id'];
        $this->addLine(
            sprintf(
                'jQuery("div#documentPane h2:nth-child(%s)").hide();'
                , $tabIndex
            )
        );
        $this->addLine('jQuery("#'.$tabId.'").hide();');
    }

    // Field
    //_______________________________________________________
    function hideField($field) {
        if(!$field) {
            return;
        }
        if($field === 'content') {
            $this->hideSection('content');
            return;
        }
        $this->addLine(
            sprintf(
                'jQuery("[name=%s]").parents("tr").hide();'
                , $field)
        );
    }

    //_______________________________________________________
    function showFields($fields) {
        $fields = explode(',', $fields);
        foreach($fields as $key=>$value) {
            $fields[$key] = trim($value);
        }
        foreach($this->fields as $field) {
            if(in_array($field,$fields)) {
                continue;
            }
            $this->hideField($field);
        }
    }
}
